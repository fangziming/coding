<%@ page contentType="text/html;charset=UTF-8" %>
<%@ include file="/WEB-INF/views/include/taglib.jsp"%>
<html>
<head>
	<title>课程管理</title>
	<meta name="decorator" content="default"/>
	<%@include file="/WEB-INF/views/include/treeview.jsp" %>
	<style type="text/css">
		.ztree {overflow:auto;margin:0;_margin-top:10px;padding:10px 0 0 10px;}
	</style>
</head>
<body>
	<sys:message content="${message}"/>
	<div id="content" class="row-fluid">
		<div id="left" class="accordion-group">
			<div class="accordion-heading">
		    	<a class="accordion-toggle">资源分类
		    		<i class="icon-refresh pull-right" onclick="refreshTree(true);"></i>
		    		<i class="icon-remove pull-right hide"></i>
		    		<i class="icon-edit pull-right hide"></i>
		    		<i class="icon-plus pull-right"></i>
		    	</a>
		    </div>
			<div id="ztree" class="ztree"></div>
		</div>
		<div id="openClose" class="close">&nbsp;</div>
		<div id="right">
			<iframe id="chapterContent"  width="100%" height="91%" frameborder="0"></iframe>
		</div>
	</div>
	<script type="text/javascript">
		var curNode=null;
		var setting = {view:{showIcon:false},data:{simpleData:{enable:true,idKey:"id",pIdKey:"pId",rootPId:'0'}},
			callback:{onClick:function(event, treeId, treeNode){
					curNode=treeNode;
					$(".icon-edit").show();
					$(".icon-remove").show();
					leftDisplay(treeNode);
				}
			}
		};
		
		function refreshTree(flag){
			if("${param.type}"!=""){
				$.getJSON("${ctx}/zxy/resourseType/jsonList/${param.type}",function(data){
					var treeObj=$.fn.zTree.init($("#ztree"), setting, data);
					treeObj.expandAll(true);
					if(flag){
						curNode=null;
						treeObj.selectNode(treeObj.getNodeByParam("id", data[0].id));
						leftDisplay(curNode);
						curNode=treeObj.getSelectedNodes()[0];
					}else{
						treeObj.selectNode(curNode);
					}
				});
			}
		}
		refreshTree(true);
		
		function leftDisplay(node){
			var url="${ctx}/zxy/";
			if("${param.type}"=="0"){
				url+="vedio";
			}else if("${param.type}"=="1"){
				url+="homework";
			}else{
				url+="quiz";
			}
			if(node!=null){
				url+="?resourseType.id="+node.id;
			}
			$("#chapterContent").attr("src",url);
		}

		$(".icon-plus").click(function(){
			$('#chapterContent').attr("src","${ctx}/zxy/resourseType/form?type=${param.type}");
		});
		
		$(".icon-remove").click(function(){
			confirmx("确认要删除分类【"+curNode.name+"】吗？",function(){
				$.getJSON("${ctx}/zxy/resourseType/delete?id="+curNode.id,function(mes){
					if(mes!=''){
						showTip(mes,"error");
					}else{
						refreshTree(true);
					}
				});
			});
		});
		
		$(".icon-edit").click(function(){
			$('#chapterContent').attr("src","${ctx}/zxy/resourseType/form?id="+curNode.id);
		});
		
		var leftWidth = 250; // 左侧窗口大小
		var htmlObj = $("html"), mainObj = $("#main");
		var frameObj = $("#left, #openClose, #right, #right iframe");
		function wSize(){
			var strs = getWindowSize().toString().split(",");
			htmlObj.css({"overflow-x":"hidden", "overflow-y":"hidden"});
			mainObj.css("width","auto");
			frameObj.height(strs[0] - 5);
			var leftWidth = ($("#left").width() < 0 ? 0 : $("#left").width());
			$("#right").width($("#content").width()- leftWidth - $("#openClose").width() -5);
			$(".ztree").width(leftWidth - 10).height(frameObj.height() - 46);
		}
	</script>
	<script src="${ctxStatic}/common/wsize.min.js" type="text/javascript"></script>
</body>
</html>