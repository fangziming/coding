...
(function ($) {
  ...
  var polyv = window.polyv = {
    ...
    fingerprint: function(file,title) {
  ...
  ResumableUpload.prototype._post = function() {
    ...
    var options = {
	  ...
	  data: {title:self.options.title,tag:self.options.tag,ext:self.options.ext,desc:self.options.desc,cataid:self.options.cataid},
      headers: {
    ...
    $.ajax(options)
      .fail(function(jqXHR, textStatus, errorThrown) {
        ...
        //self._emitFail('Could not head at file resource: ' + textStatus);
    	  localStorage.removeItem(polyv.fingerprint(self.file,self.options.title));
          self._post();
      })
      .done(function(data, textStatus, jqXHR) {
  ...
  ResumableUpload.prototype._urlCache = function(url) {
    ...
    if (fingerPrint === undefined) {
      fingerPrint = polyv.fingerprint(this.file,this.options.title);
    }
